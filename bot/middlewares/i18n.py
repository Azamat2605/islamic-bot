...
class ACLMiddleware(I18nMiddleware):
    ...
    async def get_locale(self, event: TelegramObject, data: dict[str, Any]) -> str:
        ...
        print(f"DEBUG i18n: data keys = {list(data.keys())}")
        print(f"DEBUG i18n: 'session' in data? {'session' in data}")
        print(f"DEBUG i18n: 'user' in data? {'user' in data}")
        
        # Получаем сессию из data (добавляется DatabaseMiddleware)
        session: AsyncSession | None = data.get("session")
        if session is None:
            print("DEBUG i18n: No session, returning default locale")
            ...
            if user:
                ...
                if telegram_lang:
                    print(f"DEBUG i18n: No session, using Telegram language for user {user.id}: {telegram_lang}")
                    return telegram_lang
            print("DEBUG i18n: No session and no Telegram language, returning default locale")
            return self.DEFAULT_LANGUAGE_CODE

        if hasattr(event, "chat_member"):
            print("DEBUG i18n: Chat member event, returning default locale")
            return self.DEFAULT_LANGUAGE_CODE

        user: User | None = getattr(event, "from_user", None)
        if not user:
            print("DEBUG i18n: No user, returning default locale")
            return self.DEFAULT_LANGUAGE_CODE

        # Приоритет 1: язык из базы данных
        language_code: str = await get_user_language(session=session, telegram_id=user.id)
        print(f"DEBUG i18n: Loaded locale from DB for user {user.id}: {language_code}")
        ...
        print(f"DEBUG i18n: Locale decided: {locale}")
        return locale
